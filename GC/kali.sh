...
# Start ngrok tunnel and log output
echo "Starting ngrok with region $CRP..."
./ngrok tcp --region "$CRP" 3388 > ngrok.log 2>&1 &

# Wait for ngrok to initialize
sleep 10

# Check ngrok log for issues
...
# Check if ngrok tunnel is established
if ! grep -q "URL:" ngrok.log; then
    echo "ngrok tunnel was not established. Check ngrok.log for details."
    cat ngrok.log
    exit 1
fi
